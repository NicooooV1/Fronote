...
<div style="max-width:1100px;margin:24px auto;padding:0 16px">
    <h1 style="font-size:1.5em;margin:0 0 16px"><i class="fas fa-user-shield"></i> Portail parents</h1>

    <?php if (empty($enfants)): ?>
    <p style="color:var(--text-muted,#64748b);background:#f7fafc;padding:16px;border-radius:8px">Aucun enfant rattaché à votre compte.</p>
    <?php else: ?>
    ...
    <div style="display:flex;gap:24px;flex-wrap:wrap;align-items:center;background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:18px;margin-bottom:24px">
        ...
        <div style="margin-left:auto;text-align:center">
            ...
            <div style="font-size:.8em;color:var(--text-muted,#64748b)">Moyenne générale</div>
        </div>
    </div>

    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:20px;margin-bottom:24px">
        <div>
            <h2 style="font-size:1.05em;margin:0 0 10px"><i class="fas fa-pen"></i> Dernières notes</h2>
            <?php if (empty($resume['notes_recentes'])): ?>
            <p style="color:var(--text-muted,#64748b);font-size:.9em">Aucune note récente.</p>
        ...
        <div>
            <h2 style="font-size:1.05em;margin:0 0 10px"><i class="fas fa-calendar-times"></i> Absences récentes</h2>
    ...
    <h2 style="font-size:1.1em;margin:0 0 12px"><i class="fas fa-file-signature"></i> Documents à signer (<?= count($documents) ?>)</h2>
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:24px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:8px;padding:12px 14px">
            <div>
                ...
                <?php if (!empty($d['obligatoire'])): ?><span style="background:#fef2f2;color:#991b1b;border-radius:10px;padding:2px 8px;font-size:.72em;margin-left:6px">Obligatoire</span><?php endif; ?>
                <div style="font-size:.8em;color:var(--text-muted,#64748b)">Limite : <?= htmlspecialchars($d['date_limite'] ?? '') ?></div>
            </div>
            <span style="font-size:.85em;font-weight:600;color:<?= !empty($d['deja_signe']) ? '#16a34a' : '#b45309' ?>">
                <?= !empty($d['deja_signe']) ? 'Signé' : 'À signer' ?>
            </span>
        </div>
        <?php endforeach; ?>
    </div>
    ...
    <h2 style="font-size:1.1em;margin:0 0 12px"><i class="fas fa-door-open"></i> Autorisations de sortie</h2>
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:24px">
    ...
    <h2 style="font-size:1.1em;margin:0 0 12px"><i class="fas fa-receipt"></i> Historique des paiements</h2>
    <div style="overflow-x:auto">
    <table style="width:100%;border-collapse:collapse;font-size:.9em">
        <thead><tr style="text-align:left;color:#718096;border-bottom:1px solid #e2e8f0">
            <th style="padding:8px 10px">Référence</th><th style="padding:8px 10px">Montant</th>
            <th style="padding:8px 10px">Échéance</th><th style="padding:8px 10px">Statut</th>
        </tr></thead>
        <tbody>
