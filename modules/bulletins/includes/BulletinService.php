...
class BulletinService {
    ...
    private function genererBulletinEleve(int $eleveId, int $classeId, int $periodeId, int $trimestre, ?array $statsClasse = null): bool {
        ...
        // Compter absences sur la période : test de CHEVAUCHEMENT d'intervalles (une absence à cheval
        // sur une frontière de période était auparavant ignorée par l'inclusion stricte).
        $stmt = $this->pdo->prepare("SELECT COUNT(*) FROM absences WHERE id_eleve = ? AND date_debut <= (SELECT date_fin FROM periodes WHERE id = ?) AND date_fin >= (SELECT date_debut FROM periodes WHERE id = ?)");
