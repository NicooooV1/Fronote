...
<div style="max-width:1100px;margin:24px auto;padding:0 16px">
    <h1 style="font-size:1.5em;margin:0 0 20px"><i class="fas fa-chalkboard-teacher"></i> Formation continue du personnel</h1>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:14px;margin-bottom:28px">
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            ...
            <div style="font-size:.85em;color:var(--text-muted,#64748b)">Formations (<?= htmlspecialchars($annee) ?>)</div>
        </div>
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
        ...
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            ...
            <div style="font-size:.85em;color:var(--text-muted,#64748b)">Budget consommé</div>
        </div>
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            ...
            <div style="font-size:.85em;color:var(--text-muted,#64748b)">Satisfaction moyenne</div>
        </div>
    </div>

    <?php if (!empty($expirant)): ?>
    <h2 style="font-size:1.1em;margin:0 0 12px;color:#b45309"><i class="fas fa-clock"></i> Certifications expirant bientôt (<?= count($expirant) ?>)</h2>
    ...
    <h2 style="font-size:1.1em;margin:0 0 12px"><i class="fas fa-book-open"></i> Catalogue (<?= count($catalogue) ?>)</h2>
    <?php if (empty($catalogue)): ?>
    <p style="color:var(--text-muted,#64748b);background:#f7fafc;padding:16px;border-radius:8px">Aucune formation publiée.</p>
    <?php else: ?>
    <div style="overflow-x:auto">
    <table style="width:100%;border-collapse:collapse;font-size:.9em">
        <thead><tr style="text-align:left;color:#718096;border-bottom:1px solid #e2e8f0">
            <th style="padding:8px 10px">Titre</th><th style="padding:8px 10px">Organisme</th>
            <th style="padding:8px 10px">Période</th><th style="padding:8px 10px">Places</th>
        </tr></thead>
        <tbody>
