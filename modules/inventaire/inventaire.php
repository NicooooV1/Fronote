...
<div style="max-width:1100px;margin:24px auto;padding:0 16px">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:14px;margin-bottom:28px">
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            ...
            <div style="font-size:.85em;color:var(--text-muted,#64748b)"><?= __('inventaire.valeur_achat_totale') ?></div>
        </div>
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            ...
            <div style="font-size:.85em;color:var(--text-muted,#64748b)"><?= __('inventaire.valeur_residuelle') ?></div>
        </div>
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            <div style="font-size:1.7em;font-weight:700;color:<?= $stats['pannes_ouvertes'] > 0 ? '#dc2626' : '#16a34a' ?>"><?= (int) $stats['pannes_ouvertes'] ?></div>
            <div style="font-size:.85em;color:var(--text-muted,#64748b)"><?= __('inventaire.pannes_ouvertes') ?></div>
        </div>
    </div>
    ...
    <h2 style="font-size:1.1em;margin:0 0 12px;color:#b91c1c"><i class="fas fa-exclamation-circle"></i> <?= __('inventaire.prets_en_retard') ?> (<?= count($retards) ?>)</h2>
    <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px 16px;margin-bottom:28px;font-size:.9em">
        ...
        <div><?= htmlspecialchars($r['asset_nom'] ?? '') ?> — <?= __('inventaire.retour_prevu_le') ?> <?= htmlspecialchars($r['date_retour_prevue'] ?? '') ?></div>
        <?php endforeach; ?>
    </div>
    <?php endif; ?>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:.9em">
        <thead><tr style="text-align:left;color:#718096;border-bottom:1px solid #e2e8f0">
            <th style="padding:8px 10px"><?= __('inventaire.equipement') ?></th><th style="padding:8px 10px"><?= __('label.type') ?></th>
            <th style="padding:8px 10px"><?= __('inventaire.prevue_le') ?></th>
        </tr></thead>
        <tbody>
