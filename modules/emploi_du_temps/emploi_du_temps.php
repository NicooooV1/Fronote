...
(function () {
    ...
    btn.addEventListener('click', function () {
        ...
        post('preview').then(function (d) {
            ...
            var viol = (d.score && d.score.prefs_violations != null) ? d.score.prefs_violations : '?';
            var msg = 'Aperçu : ' + d.placed_count + ' cours plaçables, ' + unplaced
                + ' besoin(s) non planifiés.\nTrous élèves : ' + trous
                + ' | Préférences non respectées : ' + viol;
            if (unplaced > 0 && d.unplaced) {
                msg += '\n\nNon planifiés :';
                d.unplaced.slice(0, 5).forEach(function (u) {
                    var r = u.requirement || {};
                    var who = 'classe ' + (r.classe_id || '?') + ' / matière ' + (r.matiere_id || '?');
                    msg += '\n• ' + who + (u.suggestion ? ' — ' + u.suggestion : '');
                });
                if (d.unplaced.length > 5) msg += '\n• … (+' + (d.unplaced.length - 5) + ')';
            }
