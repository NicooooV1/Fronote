...
<div style="max-width:1100px;margin:24px auto;padding:0 16px">
    <h1 style="font-size:1.5em;margin:0 0 20px"><i class="fas fa-globe-europe"></i> Échanges &amp; mobilité internationale</h1>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:14px;margin-bottom:28px">
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            ...
            <div style="font-size:.85em;color:var(--text-muted,#64748b)">Programmes</div>
        </div>
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            <div style="font-size:2em;font-weight:700;color:var(--primary,#0f4c81)"><?= (int) $stats['nb_participants'] ?></div>
            <div style="font-size:.85em;color:var(--text-muted,#64748b)">Participants</div>
        </div>
        <div style="background:var(--surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:16px">
            <div style="font-size:2em;font-weight:700;color:var(--primary,#0f4c81)"><?= (int) $stats['nb_partenaires_actifs'] ?></div>
            <div style="font-size:.85em;color:var(--text-muted,#64748b)">Partenaires actifs</div>
        </div>
    </div>

    <h2 style="font-size:1.1em;margin:0 0 12px"><i class="fas fa-plane-departure"></i> Programmes (<?= count($programmes) ?>)</h2>
    <?php if (empty($programmes)): ?>
    <p style="color:var(--text-muted,#64748b);background:#f7fafc;padding:16px;border-radius:8px">Aucun programme enregistré.</p>
    <?php else: ?>
    <div style="overflow-x:auto;margin-bottom:28px">
    <table style="width:100%;border-collapse:collapse;font-size:.9em">
        <thead><tr style="text-align:left;color:#718096;border-bottom:1px solid #e2e8f0">
            <th style="padding:8px 10px">Titre</th><th style="padding:8px 10px">Destination</th>
            <th style="padding:8px 10px">Période</th><th style="padding:8px 10px">Inscrits</th>
    ...
    <h2 style="font-size:1.1em;margin:0 0 12px"><i class="fas fa-handshake"></i> Partenariats actifs (<?= count($partenariats) ?>)</h2>
    <?php if (empty($partenariats)): ?>
    <p style="color:var(--text-muted,#64748b);background:#f7fafc;padding:16px;border-radius:8px">Aucun partenariat actif.</p>
    <?php else: ?>
