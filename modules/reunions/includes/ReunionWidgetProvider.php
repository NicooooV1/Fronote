 ...
 */
class ReunionWidgetProvider extends AbstractWidgetProvider
{
    public function getData(int $userId, string $userType, ?array $config = null): array
    {
        ...
        if ($userType === 'parent') {
            ...
            $stmt = $pdo->prepare(
                "SELECT r.titre, r.date_debut, r.lieu,
                 ...
                 WHERE rr.parent_id = ? AND r.date_debut >= CURDATE()
                 ORDER BY r.date_debut ASC, rc.heure_debut ASC
                 LIMIT ?"
            );
