...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        elseif ($postStep === 5) {
            ...
            // privilèges UNIQUEMENT sur sa base), on se rabat sur la suppression de toutes les
            // tables de la base existante (couvert par ALL PRIVILEGES ON `<base>`.*).
            $dbNameSafe = preg_replace('/[^a-zA-Z0-9_]/', '', $db['dbName']);
            $stmt = $pdo->prepare("SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = ?");
            $stmt->execute([$db['dbName']]);
            $dbExists = (bool) $stmt->fetch();

            $dropAllTables = static function (PDO $pdo, string $schema): void {
                $pdo->exec("USE `{$schema}`");
                $pdo->exec('SET FOREIGN_KEY_CHECKS=0');
                $objs = $pdo->query("SELECT TABLE_NAME, TABLE_TYPE FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{$schema}'")->fetchAll(PDO::FETCH_ASSOC);
                foreach ($objs as $o) {
                    $kind = (($o['TABLE_TYPE'] ?? '') === 'VIEW') ? 'VIEW' : 'TABLE';
                    try { $pdo->exec("DROP {$kind} IF EXISTS `{$o['TABLE_NAME']}`"); } catch (PDOException $e) {}
                }
            ...
            if ($dbExists) {
                ...
                $tableCount = (int) $pdo->query("SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{$dbNameSafe}'")->fetchColumn();
                if ($tableCount > 0 && empty($_POST['confirm_overwrite'])) {
                    $inst['db_exists'] = true;
            ...
            if (file_exists($bootstrapFile)) {
                try {
                    ...
                    try {
                        ...
                        // Miroir TENANT de l'administrateur (compte staff + appartenance + rôle administration).
                        ...
                        $tId = $tExist ? (int) $tExist['id'] : $tas->createAccount([
                            'account_type' => 'staff', 'username' => 'admin', 'email' => $adm['mail'],
                            'password_hash' => $hash, 'first_name' => $adm['prenom'], 'last_name' => $adm['nom'],
                        ...
                        // Super-admin PLATEFORME (identifiant « superadmin » ; mêmes identifiants que l'admin).
                        $pas = new \API\Platform\PlatformAccountService($pdo);
                        if (!$pas->findActiveByLogin('superadmin')) {
                            $paId = $pas->createAccount([
                                'email' => $adm['mail'], 'username' => 'superadmin', 'password_hash' => $hash,
                                'first_name' => $adm['prenom'], 'last_name' => $adm['nom'], 'status' => 'active',
                            ]);
